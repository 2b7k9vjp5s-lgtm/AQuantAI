...
- `python -m scripts.probe_hithink_contracts`: run the credential-safe Hithink contract acceptance probe. Use `--offline-contract` for synthetic, network-free validation. Live mode requires explicit `--allow-network` and reads `HITHINK_FINANCE_API_KEY` only from the account owner's local environment; it performs no database writes or dump download.

The Hithink probe is not a provider adapter or ingestion command. See `docs/hithink_contract_probe.md` for its exact representatives, local evidence and sanitized-output contract.
