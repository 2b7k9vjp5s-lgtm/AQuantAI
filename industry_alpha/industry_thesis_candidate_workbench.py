...
def _visible(cutoff: date, recorded_at: datetime, *, as_of_cutoff: date, boundary: datetime) -> bool:
...
class IndustryThesisCandidateWorkbenchService:
    ...
    def candidate_source_options(
        ...
        *,
        ...
        return {
            ...
            "is_exact_latest_revision": identity.latest_revision_number == revision.revision_number,
            "build_allowed": (
    ...
    def compose_candidate_build(
        ...
        *,
        ...
        if expected_session_latest_revision_number != options["session_revision_number"]:
            raise _error(
                "industry_thesis_revision_conflict",
        ...
        proposals = [self._seed_proposal(item, session_revision_id) for item in options["company_seeds"]]
        stage1_count = 0
        ...
        command = {
            ...
            "expected_session_latest_revision_number": expected_session_latest_revision_number,
        ...
        summary = {
            ...
            "selected_candidate_pool_revision_ids": [str(value) for value in sorted(selected, key=str)],
    ...
    def _exact_session_revision(
        ...
        *,
        ...
        if identity is None or stored_utc(identity.created_recorded_utc) > boundary:
            raise _error("industry_thesis_session_not_found", "exact session was not found")
        if not _visible(
            ...
            raise _error("industry_thesis_not_visible", "session revision is outside the requested boundary")
        return identity, revision

    def _scope_values(self, revision: IndustryThesisSessionRevision) -> tuple[list[Any], list[Any]]:
        ...
        if not isinstance(seeds, list) or not isinstance(graph, dict):
            raise _error("industry_thesis_graph_incomplete", "stored workbench scope is not structured")
        ...
        if not isinstance(maps, list):
            raise _error("industry_thesis_graph_incomplete", "stored map references are incomplete")
        return seeds, maps
    ...
    def _company_seeds(
        ...
        for index, item in enumerate(seeds):
            require_keys(item, allowed, allowed, field=f"seed_companies[{index}]")
            ...
            if stock_id is not None:
                ...
                run = None if stock is None else self._session.get(IngestionRun, stock.ingestion_run_id)
                if stock is None or run is None or run.status != "succeeded" or run.completed_at is None:
                    raise _error("industry_thesis_identity_not_found", "exact stock seed was not found")
                if run.information_cutoff_date > as_of_cutoff or stored_utc(run.completed_at) > boundary:
                    raise _error("industry_thesis_later_information", "stock seed exceeds the requested boundary")
                if str(stock.id) != str(item["exact_id"]):
                    raise _error("industry_thesis_identity_invalid", "stock seed exact ID does not match")
                exact_key = ("stock_basic_record", str(stock.id))
            else:
                instrument_uuid = parse_uuid(instrument_id, "listed_instrument_id")
                instrument = self._session.get(ListedInstrument, instrument_uuid)
                if instrument is None or stored_utc(instrument.created_at_utc) > boundary:
                    raise _error("industry_thesis_identity_not_found", "exact listed-instrument seed was not found")
                visible = self._session.scalar(
                    select(ListedInstrumentRevision)
                    .where(
                        ListedInstrumentRevision.instrument_id == instrument.id,
                        ListedInstrumentRevision.information_cutoff_date <= as_of_cutoff,
                        ListedInstrumentRevision.recorded_at_utc <= boundary,
                    )
                    .order_by(ListedInstrumentRevision.revision_no.desc())
                    .limit(1)
                )
                if visible is None or str(instrument.id) != str(item["exact_id"]):
                    raise _error("industry_thesis_identity_not_found", "listed-instrument seed is not visible")
                exact_key = ("listed_instrument", str(instrument.id))
            if exact_key in seen:
                raise _error("industry_thesis_duplicate_source", "stored scope contains a duplicate exact company seed")
            seen.add(exact_key)
            result.append(
                {
                    "source_kind": exact_key[0],
                    "exact_id": exact_key[1],
                    "stock_basic_record_id": None if stock_id is None else int(stock_id),
                    "listed_instrument_id": None if instrument_id is None else str(instrument_id),
        ...
        result.sort(key=lambda item: (item["label"].casefold(), item["code"].casefold(), item["source_kind"], item["exact_id"]))
        return result
    ...
    def _map_source_options(
        ...
        seen: set[UUID] = set()
        allowed = {"source_kind", "map_id", "map_revision_id", "revision_number", "title"}
        for index, item in enumerate(references):
            require_keys(item, allowed, allowed, field=f"exact_industry_map_references[{index}]")
            map_revision_id = parse_uuid(item["map_revision_id"], "map_revision_id")
            if map_revision_id in seen:
                raise _error("industry_thesis_duplicate_source", "stored scope contains a duplicate map revision")
            seen.add(map_revision_id)
            map_revision = self._session.get(IndustryMapRevision, map_revision_id)
            if map_revision is None or str(map_revision.map_id) != str(item["map_id"]):
                raise _error("industry_thesis_source_not_found", "exact selected Industry Map revision was not found")
            if map_revision.revision_no != int(item["revision_number"]):
                raise _error("industry_thesis_source_invalid", "stored Industry Map revision number does not match")
            if not _visible(
                ...
                raise _error("industry_thesis_later_information", "Industry Map revision exceeds the requested boundary")
            pools = self._eligible_pools(map_revision, as_of_cutoff, boundary)
            maps.append(
                {
                    ...
                    "availability_state": "available" if pools else "no_exact_frozen_pool",
                }
            )
        maps.sort(key=lambda item: (item["title"].casefold(), item["industry_map_revision_id"]))
        return maps
    ...
    def _eligible_pools(
        ...
        rows = list(
            self._session.execute(
                ...
                .join(
                    Stage1CandidatePoolRevision,
                    Stage1CandidatePoolRevision.candidate_pool_id == Stage1CandidatePool.id,
                )
                .where(
                    Stage1CandidatePool.map_id == map_revision.map_id,
                    Stage1CandidatePool.created_at_utc <= boundary,
                    Stage1CandidatePoolRevision.selected_map_revision_id == map_revision.id,
            ...
            result.append(
                {
                    ...
                    "information_cutoff_date": pool_revision.information_cutoff_date.isoformat(),
                    "recorded_at_utc": stored_utc(pool_revision.recorded_at_utc).isoformat(),
                    "selected": False,
                }
            )
        return result
    ...
    def _pool_members(
        ...
        *,
        ...
        pool_revision = self._session.get(Stage1CandidatePoolRevision, pool_revision_id)
        map_revision = self._session.get(IndustryMapRevision, map_revision_id)
        if pool_revision is None or map_revision is None:
            raise _error("industry_thesis_source_not_found", "exact frozen candidate-pool source was not found")
        pool = self._session.get(Stage1CandidatePool, pool_revision.candidate_pool_id)
        if pool is None or pool.map_id != map_revision.map_id or pool_revision.selected_map_revision_id != map_revision.id:
            raise _error("industry_thesis_source_invalid", "candidate pool is not bound to the selected map revision")
        if not _visible(pool_revision.information_cutoff_date, pool_revision.recorded_at_utc, as_of_cutoff=as_of_cutoff, boundary=boundary):
            raise _error("industry_thesis_later_information", "candidate-pool revision exceeds the requested boundary")
        memberships = list(
        ...
        for membership in memberships:
            revision = self._session.get(Stage1BeneficiaryRevision, membership.beneficiary_revision_id)
            beneficiary = self._session.get(Stage1Beneficiary, membership.beneficiary_id)
            if revision is None or beneficiary is None or revision.beneficiary_id != beneficiary.id:
                raise _error("industry_thesis_graph_incomplete", "frozen Stage 1 membership graph is incomplete")
            if revision.selected_map_revision_id != map_revision.id or beneficiary.map_id != map_revision.map_id:
                raise _error("industry_thesis_source_invalid", "Stage 1 beneficiary is not bound to the exact map revision")
            if revision.assessment_status != "supported":
                raise _error("industry_thesis_source_invalid", "frozen candidate-pool member is not a supported Stage 1 revision")
            if not _visible(revision.information_cutoff_date, revision.recorded_at_utc, as_of_cutoff=as_of_cutoff, boundary=boundary):
                raise _error("industry_thesis_later_information", "Stage 1 beneficiary revision exceeds the requested boundary")
            stock = self._session.get(StockBasicRecord, revision.stock_basic_record_id)
            run = None if stock is None else self._session.get(IngestionRun, stock.ingestion_run_id)
            if stock is None or run is None or run.status != "succeeded" or run.completed_at is None:
                raise _error("industry_thesis_identity_not_found", "Stage 1 company snapshot was not found")
            if stock.source != beneficiary.source or stock.stock_code != beneficiary.stock_code:
                raise _error("industry_thesis_identity_invalid", "Stage 1 company snapshot does not match beneficiary identity")
            if run.information_cutoff_date > as_of_cutoff or stored_utc(run.completed_at) > boundary:
                raise _error("industry_thesis_later_information", "Stage 1 company snapshot exceeds the requested boundary")
            result.append(
                {
                    "industry_map_revision_id": str(map_revision.id),
                    "candidate_pool_revision_id": str(pool_revision.id),
        ...
        result.sort(key=lambda item: (item["beneficiary_kind"], item["stock_code"], item["candidate_pool_membership_id"]))
        return result

    @staticmethod
    def _seed_proposal(seed: dict[str, Any], session_revision_id: UUID) -> dict[str, Any]:
        proposal = {
            ...
            "benefit_path_text": "该公司由用户在研究范围确认时明确选择为精确公司种子；尚未审阅其受益路径。",
        ...
        if seed["stock_basic_record_id"] is not None:
            proposal["proposed_stock_basic_record_id"] = seed["stock_basic_record_id"]
        else:
            proposal["proposed_listed_instrument_id"] = seed["listed_instrument_id"]
        return proposal
    ...
    def _stage1_proposal(member: dict[str, Any]) -> dict[str, Any]:
        return {
            ...
            "proposed_stock_basic_record_id": member["stock_basic_record_id"],
            "company_label_original": member["company_label"],
            "industry_position": f"stage1_beneficiary_kind:{member['beneficiary_kind']}",
...
class IndustryThesisWorkbenchCandidateCommandService(IndustryThesisCommandService):
    ...
    def _validate_candidate_source(
        ...
        if proposal["identity_state"] != "exact_accepted_identity":
            raise _error("industry_thesis_identity_invalid", "frozen Stage 1 source requires exact identity")
        stock_id = proposal["proposed_stock_basic_record_id"]
        if stock_id is None or proposal["proposed_listed_instrument_id"] is not None:
            raise _error("industry_thesis_identity_invalid", "frozen Stage 1 source requires one exact stock identity")
        source_reference = json_value(proposal["source_reference_json"], "source_reference")
        require_keys(source_reference, _STAGE1_SOURCE_KEYS, _STAGE1_SOURCE_KEYS, field="source_reference")
        map_revision = session.get(
            IndustryMapRevision,
            parse_uuid(source_reference["industry_map_revision_id"], "industry_map_revision_id"),
        )
        pool_revision = session.get(
            Stage1CandidatePoolRevision,
            parse_uuid(source_reference["candidate_pool_revision_id"], "candidate_pool_revision_id"),
        )
        membership = session.get(
            Stage1CandidatePoolMembership,
            parse_uuid(source_reference["candidate_pool_membership_id"], "candidate_pool_membership_id"),
        )
        beneficiary_revision = session.get(
            Stage1BeneficiaryRevision,
            parse_uuid(source_reference["stage1_beneficiary_revision_id"], "stage1_beneficiary_revision_id"),
        )
        if None in {map_revision, pool_revision, membership, beneficiary_revision}:
            raise _error("industry_thesis_source_not_found", "exact frozen Stage 1 source graph was not found")
        pool = session.get(Stage1CandidatePool, pool_revision.candidate_pool_id)
        beneficiary = session.get(Stage1Beneficiary, beneficiary_revision.beneficiary_id)
        ...
        if pool is None or beneficiary is None or stock is None or run is None or run.completed_at is None:
            raise _error("industry_thesis_graph_incomplete", "exact frozen Stage 1 source graph is incomplete")
        if (
            ...
            or beneficiary_revision.selected_map_revision_id != map_revision.id
            ...
            raise _error("industry_thesis_source_invalid", "exact frozen Stage 1 source bindings do not match")
        if beneficiary_revision.assessment_status != "supported":
            raise _error("industry_thesis_source_invalid", "frozen Stage 1 candidate is not supported")
        dated_rows = (
            (map_revision.information_cutoff_date, map_revision.recorded_at_utc),
            (pool_revision.information_cutoff_date, pool_revision.recorded_at_utc),
            (beneficiary_revision.information_cutoff_date, beneficiary_revision.recorded_at_utc),
            (run.information_cutoff_date, run.completed_at),
        )
        if any(item_cutoff > cutoff or stored_utc(item_recorded) > recorded_at for item_cutoff, item_recorded in dated_rows):
            raise _error("industry_thesis_later_information", "frozen Stage 1 source exceeds the thesis boundary")
        if stored_utc(membership.recorded_at_utc) > recorded_at:
            raise _error("industry_thesis_later_information", "candidate-pool membership exceeds the thesis boundary")
