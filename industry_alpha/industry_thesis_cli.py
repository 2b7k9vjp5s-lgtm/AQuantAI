"""Local bounded JSON command runner for offline Industry Thesis Orchestration v1."""
...
def run(method_name: str, description: str, argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        engine = build_engine()
        service = IndustryThesisCommandService(build_session_factory(engine))
            ...
            json.dumps(
                {
                    ...
                    "error": {
                        ...
                        "message": "Local industry-thesis command failed. Verify input and database configuration.",
                    },
                },
                ensure_ascii=True,
                sort_keys=True,
