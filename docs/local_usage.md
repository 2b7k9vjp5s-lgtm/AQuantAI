...
Benchmark-index normalization and persistence are separate from equity ingestion. Offline dry-run:

```bash
python -m scripts.ingest_akshare_benchmark_data \
  --index-code 000001 \
  --start-date 20260105 \
  --end-date 20260403 \
  --cutoff 20260405 \
...
Live use replaces `--offline-fixture` with `--allow-network`, requires cutoff to equal the UTC collection date, and accepts at most 20 explicit codes. It uses only `index_zh_a_hist`, with finite timeout/retry behavior and no default code or endpoint fallback.

Selected-sector normalization and persistence are a third, separate domain. Offline dry-run:

```bash
python -m scripts.ingest_akshare_sector_data \
  --sector-code BK0001 \
  --sector-code BK0002 \
  --start-date 20260105 \
  --end-date 20260403 \
...
Optional benchmark context uses an independently persisted series:

```text
http://127.0.0.1:8000/market-cockpit/snapshot?series_key=<equity-series>&benchmark_series_key=<benchmark-series>&as_of_cutoff=YYYYMMDD
http://127.0.0.1:8000/market-cockpit?series_key=<equity-series>&benchmark_series_key=<benchmark-series>&as_of_cutoff=YYYYMMDD
```

Without `benchmark_series_key`, benchmark context remains visibly unavailable and equity behavior is unchanged. An invalid or unavailable benchmark key returns 422 or 404 rather than falling back or guessing.

Optional selected-sector context uses another independently persisted series:

```text
http://127.0.0.1:8000/market-cockpit/snapshot?series_key=<equity-series>&sector_series_key=<sector-series>&as_of_cutoff=YYYYMMDD
http://127.0.0.1:8000/market-cockpit?series_key=<equity-series>&sector_series_key=<sector-series>&as_of_cutoff=YYYYMMDD
```

Without `sector_series_key`, sector context is `null` and existing equity/benchmark behavior is unchanged. The sector section is selected-scope, provider-attributed, non-official, descriptive, read-only, and non-advisory. It does not contain constituents, company beneficiaries, Industry Alpha conclusions, scores, signals, or recommendations.
...
python -m scripts.demo_benchmark_context
...
Run the fully offline v0.5B evidence-backed industry-chain-map fixture demo:

```bash
python -m scripts.demo_industry_chain_map
```

The map demo compares the current frozen map with the earlier `2026-07-03` cutoff. The current view includes supported A/B/C-backed assertions, a D-grade lead that remains draft, and one visible disputed conflict. It performs no network call and does not write the configured PostgreSQL database.
...
GET http://127.0.0.1:8000/industry-alpha/maps
GET http://127.0.0.1:8000/industry-alpha/maps/{map_id}
GET http://127.0.0.1:8000/industry-alpha/maps/{map_id}?as_of_cutoff=YYYY-MM-DD
```

There are no HTTP create, update, or delete routes. A missing or cutoff-invisible case or map returns 404, malformed dates return 422, and missing database configuration or schema returns 503. See [Industry Alpha evidence ledger](industry_alpha_evidence_ledger.md) and [evidence-backed industry chain maps](industry_chain_maps.md) for the append-only, evidence, conflict, and historical visibility rules.
...
- v0.5B records only evidence-bound, versioned industry-chain structure and observations. It adds no scoring, company beneficiaries, Stage 2 research, LLM execution, recommendations, or trading.
