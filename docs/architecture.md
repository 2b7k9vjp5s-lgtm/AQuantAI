...
## v0.4A Market Cockpit Boundary

v0.4A introduces a separate selected-universe Market Cockpit module with contracts, deterministic calculator, single-run repository adapter, application service, lazy FastAPI dependency, read-only JSON endpoint, and local static page. It selects one successful complete snapshot by explicit series key or complete canonical selector and reads all source rows by the selected ingestion-run ID. It never uses provider-only selection, combines runs/series, changes adjustment policy, starts an engine during import/startup, triggers collection, or falls back to fixture Dashboard data.

Market Cockpit keeps calculation readiness separate from scope coverage confidence. v0.4A has no reviewed representative-coverage policy, so scope coverage is explicitly unverified and overall completeness remains conservative even when calculations are internally ready. The view exposes only an allowlist of immutable ingestion and canonical compatibility provenance; opaque request metadata is never serialized. A zero-volume and zero-amount row is treated as no-trade and excluded consistently from latest breadth, participation, rolling close, and risk windows, with bounded stale/no-trade diagnostics.

The calculator uses only persisted stock basic, daily price, and trade calendar data. Official indices, sectors, style, valuation/market-cap breadth, and crowding remain unsupported. The existing fixture Dashboard and Quant Core behavior remain unchanged. No scheduler, automatic refresh, Industry Alpha, LLM, watchlist, simulated portfolio, broker, order, or trading behavior is added.
