...
Every amount sum uses one fail-closed finite aggregate operation. It rejects arithmetic overflow and any non-finite result even when every individual amount is finite and positive. When the latest total is invalid, `latest_total_amount` and both concentration shares are `null`, exact eligible/unavailable counts remain unchanged, `latest_aggregate_reason` is `non_finite_aggregate`, and liquidity status cannot be `complete`. Top-5 and top-decile numerators are validated independently and a share is available only when both numerator and denominator are finite and the denominator is positive.
...
Every matched-cohort session total uses the same fail-closed aggregate operation. If any required session total overflows or is non-finite, latest total, baseline, and ratio for that window are all `null`, status is `unavailable`, and reason is `non_finite_aggregate`. The response never retains a partial total or ratio from the remaining sessions.
